...
let urlLauncher = urlInstance + "Launcher/";
let urlAPI = "http://useritium.fr/Download/app/index.php"

function createWindow () {
    mainWindow = new BrowserWindow({
        frame: false,
        title: "AuroraCraft Loader",
        width: 419,
        height: 572,
        resizable: false,
...
ipcMain.on("start", (event, data) =>{
    ...
    let AuroraInstance = path.join(app.getPath("appData"), urlInstance);
    let LauncherInstance = path.join(app.getPath("appData"), urlLauncher);
    ...
    * */
    ...
    // Si il existe déjà un AuroraCraft
    if (!fs.existsSync(AuroraInstance)) {
        console.log("a creer");

        fs.mkdir(AuroraInstance, (err) => {
            if (err) {
                showErrorDialog("Erreur", 'Une erreur s\'est produite. Veuillez réessayer.');
            } else {
                console.log("Repertoire '.auroracraft' cree avec succes.");
            }
        });
    ...
    // Si il existe déjà auroracraft/launcher
    if (!fs.existsSync(LauncherInstance)){

        fs.mkdir(LauncherInstance, (err) => {
            if (err) {
                showErrorDialog("Erreur", 'Une erreur s\'est produite. Veuillez réessayer.');
            } else {
    ...
    * */
    ...
    getInfoLoaderServer(urlAPI)
        .then(InfoLoaderServer => {
            ...
            let pathLaunch = path.join(app.getPath("appData"), '/.AuroraCraft/Launcher/' + versionName +'/');
    ...
    * */
    ...
    function lancement(versionName){
        ...
        const executablePath = LauncherInstance + versionName + '/auroracraft-launcher.exe';
